...
public class MazeViewer {
	...
	private static MazeFrame readMazeFile(String fileName) throws IOException {
		...
		int rowvalue = in.nextInt();
		int colvalue = in.nextInt();

		boolean[][] maze = new boolean[rowvalue][colvalue];
		...
		for (int i = 0; i < maze.length; i++) {
			...
			for (int j = 0; j < maze[0].length; j++) {
				...
				char wallCheck = abc.charAt(j);

				if (wallCheck == WALL_CHAR) {
					maze[i][j] = false;
				} else {
					maze[i][j] = true;
				}
		...
		int startloc1 = in.nextInt();
		int startloc2 = in.nextInt();
		int endloc1 = in.nextInt();
		int endloc2 = in.nextInt();

		MazeCoord startLoc1 = new MazeCoord(startloc1, startloc2);
		MazeCoord endLoc1 = new MazeCoord(endloc1, endloc2);

		return new MazeFrame(maze, startLoc1, endLoc1);
